...
module Representable::Binding::Deprecation
  ...
  private
    def evaluate_option_with_deprecation(binding, name, input, options, *positional_arguments)
      ...
      __options = if binding[:pass_options]
  ...
  Learn more here: http://trailblazerb.org/gems/representable/upgrading-guide.html#pass-options}
        Options.new(binding, options[:user_options], binding.represented, binding.instance_variable_get(:@parent_decorator))
      else
        # user_options
      ...
      if proc.send(:proc?) or proc.send(:method?)
        ...
        arity = binding.send(:exec_context).method(proc.instance_variable_get(:@value)).arity if proc.send(:method?)
  ...
  Learn more here: http://trailblazerb.org/gems/representable/upgrading-guide.html#positional-arguments
          ...
          return proc.(binding.send(:exec_context), *deprecated_args)
        end
      end
